...
PATH = os.path.join(SAVE_DIR, f"cubic_model_{timestamp}.pth")  # model/quadratic_model.pth
...
# def train(model, loss_function, optimizer, scheduler, train_dataloader, x_eval, y_eval, epochs):
def train(model, loss_function, optimizer, train_dataloader, x_eval, y_eval, epochs):
    print('삼차 함수 학습 시작')
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1)%200==0:
            ...
            # print(f"\r[EPOCH {epoch+1}/{epochs}] Train Loss: {avg_loss:.6f}, Eval Loss: {eval_loss:.6f}, 현재 학습률: {scheduler.get_last_lr()[0]}")
            print(f"\r[EPOCH {epoch+1}/{epochs}] Train Loss: {avg_loss:.6f}, Eval Loss: {eval_loss:.6f}")

    torch.save(model, PATH)
    print("삼차 함수 모델 저장 완료:", PATH)

    return train_loss_history

def loss_function_(outputs, targets):
    result = torch.sum((outputs-targets)**2)/len(targets)
    return result
...
def visualize_results(epochs, loss_history, inputs1, targets1, predictions1, inputs2, targets2, predictions2):
    ...
    axs[0].set_title("[Cubic] Training Loss")
    # axs[0].set_ylim(-0.01, 10.0)
    axs[0].legend()
    axs[0].grid(True)
    ...
    axs[1].set_title("[Cubic] TEST [-10,10]")
    axs[1].legend()
    axs[1].grid(True)
    ...
    axs[2].set_title("[Cubic] TEST [-20,-10]")
    axs[2].legend()
    axs[2].grid(True)
...
if __name__ == "__main__":
    ...
    # 4) 학습
    # loss_history = train(model, loss_function, optimizer, scheduler, train_dataloader, x_eval, y_eval, EPOCHS)
